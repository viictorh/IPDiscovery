...
/**
 * Gets information about network interface given a jpcap device string, on
 * Windows. Makes use of WinRegistry class from
 * http://stackoverflow.com/a/6163701/616460. This is tested against jpcap
 * 0.01.16, which is available for download at
 * http://sourceforge.net/projects/jpcap/.
 * 
 * All getters return empty strings rather than null if the information is
 * unavailable.
 * 
...
public class NetworkDeviceInfo {
	 ...
	 * 
	 * @param jpcapDeviceString
	 *            Device string from jpcap.
	 * @throws IllegalArgumentException
	 *             If the device string could not be parsed.
	 * @throws UnsupportedOperationException
	 *             If the Windows registry could not be read.
	 */
	public NetworkDeviceInfo(String jpcapDeviceString) throws IllegalArgumentException, UnsupportedOperationException {

		// extract jpcap device and display name, and guid, from jpcap device
		// string
		...
		try {
			...
			// Search all subkeys of subkeys in
			// HKLM\SYSTEM\CurrentControlSet\Control\Class\. If a subkey
			// is found that contains a key NetCfgInstanceId whose value is
			// {guid}, then the rest of the keys
			// there will contain driver info - the nice display name, vendor
			// info, etc.
			...
			for (String driverClassSubkey : WinRegistry.readStringSubKeys(DRIVER_CLASS_ROOT, DRIVER_CLASS_PATH)) {
				for (String driverSubkey : WinRegistry.readStringSubKeys(DRIVER_CLASS_ROOT,
						DRIVER_CLASS_PATH + "\\" + driverClassSubkey)) {
			...
			// read tcp/ip configuration details
			// (HKLM\SYSTEM\CCS\services\Tcpip\Parameters\Interfaces\{guid})
			// there is an integer key EnableDHCP, but
			// java.util.prefs.WindowsPreferences (and therefore
			// WinRegistry) supports reading string keys only, therefore we'll
			// have to hack it to decide on
			// DHCP vs. static IP address and hope it's correct.
			// also note the ip addresses are REG_MULTI_SZ, presumably to also
			// hold ipv6 addresses. the results
			// here may not be quite correct, then. that's why I'm leaving
			// addresses as strings instead of
	 ...
	 * @param def
	 *            A default string.
	 * @return Returns def if str is null or empty (after trim), otherwise
	 *         returns str, trimmed.
	 */
	 ...
	 * 
	 * @return Device name from jpcap. Pass this string to PacketCapture to
	 *         specify this device.
	 */
	 ...
	 * @bug This may not be correct for interfaces with multiple IP addresses.
	 *      For this reason, it is left as a raw string rather than being
	...
	public String toString() {
		return String.format("%s (%s) {%s} @ %s/%s", driverName, driverVendor, guid, interfaceAddress,
				interfaceSubnetMask);
	}
